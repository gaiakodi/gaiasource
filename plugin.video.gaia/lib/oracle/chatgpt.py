...
class Chatgpt(Oracle):
	...
	def _chat(self, message, context = None, conversation = None, refine = None, media = None):
		...
		if not result or (result['error'] and not any(result['chat']['history'][-1]['data'])): result = self._requestPlayground(message = message, context = context, conversation = conversation, refine = refine, model = model, limit = limit, randomness = randomness, creativity = creativity)
		return result
