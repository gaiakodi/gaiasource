...
class Interface(base.Interface):
	...
	def add(self, link, name = None, title = None, year = None, season = None, episode = None, pack = False, strict = False, close = True, type = None, cached = False, select = False, cloud = False):
		if cloud: interface.Loader.show()

		result = self.mDebrid.add(link = link, name = name, title = title, year = year, season = season, episode = episode, pack = pack, strict = strict, type = type, cached = cached, cloud = cloud)
		if select: result = self._addSelect(result, type = type)
	...
	def _addSelect(self, result, type = None):
		try:
			...
			for i in items:
				...
				try:
					if len(i['parent']['parts']) > 1:
						path = [interface.Format.fontItalic(j) for j in i['parent']['parts'][1:]]
			...
			if choice < 0:
				...
				# Only delete if it is not a new download (aka not already exists). Otherwise packs might be incorrectly deleted.
				if result['new'] and (type is None or self.mDebrid.deletePossible(type)): self.mDebrid.deleteSingle(id = result['id'], wait = False)
			else:
	...
	def _addLink(self, result, title = None, year = None, season = None, episode = None, close = True, pack = False, strict = False, type = None, select = False):
		...
		self.tLink =  ''
		item = self.mDebrid.item(idTransfer = id, content = True, title = title, year = year, season = season, episode = episode, pack = pack, strict = strict)
		if select: item = self._addSelect(item, type = type)
		if item:
			try:
			...
			if self._addErrorDetermine(item, pack = pack):
									...
									if percentageNew >= percentage:
										...
										if background:
											...
											if speed:
												...
												if peers == 0: peers = ''
												else: peers = ' from ' + str(peers) + ' peers'
				...
				try:
					thread = Pool.thread(target = updateProgress, args = (id, percentage, close))
					thread.start()
					thread.join()
				except:
					tools.Logger.error()
	...
	def directoryItemAction(self, item):
		...
		if choice >= 0:
			...
			elif choice == 1: networker.Networker.linkShow(link = link)
			elif choice == 2: networker.Networker.linkOpen(link = link)
				...
				try:
					...
					if choice == 'remove':
						...
						if choice == 'browsecontent':
							...
							if network.Networker.linkIs(itemLink): networker.Networker.linkShow(link = itemLink)
							else: raise Exception('Invalid Largest Link: ' + str(itemLink))
						elif choice == 'openlargest':
							if network.Networker.linkIs(itemLink): networker.Networker.linkOpen(link = itemLink)
							else: raise Exception('Invalid Largest Link: ' + str(itemLink))
						else:
							...
							if choice == 'downloadzip':
								...
								if network.Networker.linkIs(itemZip): networker.Networker.linkShow(link = itemZip)
								else: raise Exception('Invalid ZIP Link: ' + str(itemZip))
							elif choice == 'openzip':
								if network.Networker.linkIs(itemZip): networker.Networker.linkOpen(link = itemZip)
								else: raise Exception('Invalid ZIP Link: ' + str(itemZip))
				except:
					tools.Logger.error()
