...
from lib.debrid.realdebrid.core import Core
from lib.debrid.realdebrid.interface import Interface
...
class Handle(HandleBase):
	...
	def handle(self, link, item, download = False, popups = False, close = True, select = False, cloud = False, strict = False):
		if self.mService.accountValid():
			...
			cached = stream.accessCacheRealdebrid()
	...
	def enabled(self, type):
		return type in [Core.ModeTorrent, Core.ModeHoster] and self.mService.streaming(type) and self.mService.accountValid()

	def supported(self, item, cloud = False):
		try: type = item['stream'].sourceType()
		except: type = item
		if type == Core.ModeTorrent: return True
