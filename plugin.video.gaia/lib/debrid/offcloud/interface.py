...
class Interface(base.Interface):
	...
	def account(self):
		...
		if self.mDebrid.accountEnabled():
			...
			if account:
				...
				none = interface.Translation.string(33112)
				yes = interface.Translation.string(33341)
				no = interface.Translation.string(33342)

				valid = yes if self.mDebrid.accountValid() else no
				premium = yes if account['premium'] else no
				date = account['expiration']['date'] if account['expiration']['date'] else unlimited
				days = str(account['expiration']['remaining']) if account['expiration']['remaining'] else unlimited

				limitsLinks = unlimited if account['limits']['links'] <= 0 else str(account['limits']['links'])
				limitsPremium = unlimited if (account['limits']['premium'] <= 0 and account['premium']) else none if account['limits']['premium'] <= 0 else str(account['limits']['premium'])
				limitsTorrents = unlimited if account['limits']['torrents'] <= 0 else str(account['limits']['torrents'])
				limitsStreaming = unlimited if account['limits']['streaming'] <= 0 else str(account['limits']['streaming'])
	...
	def settingsLocation(self, settings = True):
		...
		proxy = None
		choice = interface.Dialog.select(title = 35207, items = proxiesNames)
		if choice >= 0:
			if choice == 0: proxy = {'id' : None, 'name' : default}
			else: proxy = proxies[choice - 1]
		self.mDebrid.instantUpdate(proxy, settings = settings)
	...
	def add(self, link, category = None, name = None, title = None, year = None, season = None, episode = None, pack = False, strict = False, close = True, type = None, cached = None, cloud = False, select = False):
		if cloud: interface.Loader.show()
		result = self.mDebrid.add(link = link, category = category, name = name, title = title, year = year, season = season, episode = episode, pack = pack, strict = strict, type = type)
		if select: result = self._addSelect(result, category = category, type = type)
		...
		if result['success']:
			...
			title = 'Stream Error'
			message = 'OffCloud Stream Unavailable'
			...
			return self._addLink(result = result, title = title, year = year, season = season, episode = episode, close = close, pack = pack, strict = strict, cached = cached, type = type, cloud = False, select = select)
		else:
	...
	def _addSelect(self, result, category = None, type = None):
		try:
			if not result: return result
			try: items = [i for i in result['items']['files'] if i['name'] and not i['name'].endswith(core.Core.Exclusions)]
			except:
				try: items = [i for i in result['files'] if i['name'] and not i['name'].endswith(core.Core.Exclusions)]
				except: return result # Still busy downloading.
			items = sorted(items, key = lambda x : x['name'])
			choice = interface.Dialog.select(title = 35542, items = [interface.Format.fontLight('') + i['name'] for i in items]) # Add light font for consistent formatting with other debrids.
			if choice < 0:
				result['success'] = False
				result['error'] = core.Core.ErrorSelection
				if type is None or self.mDebrid.deletePossible(type): self.mDebrid.delete(id = result['id'], category = category, wait = False)
			else:
	...
	def _addDelete(self, category, id, notification = False):
		...
		thread = Pool.thread(target = __addDelete, args = (category, id, notification))
		thread.start()
	...
	def _addLink(self, result, title = None, year = None, season = None, episode = None, close = True, pack = False, strict = False, cached = None, type = None, cloud = False, select = False):
		...
		self.tLink =  ''
		item = self.mDebrid.item(category = category, id = id, title = title, year = year, season = season, episode = episode, pack = pack, strict = strict, transfer = True, files = True)
		if select: result = self._addSelect(result, category = category, type = type)
		if item:
			try:
			...
			if self._addErrorDetermine(item, pack = pack, category = category):
				...
				def updateProgress(status, category, id, percentage, close, cached):
					...
					while True:
						...
						if processing:
							...
							created = True
							if cloud: interface.Loader.hide()
							interface.Core.create(type = interface.Core.TypeDownload, title = heading, message = descriptionWaiting)
							interface.Core.update(progress = int(percentage), title = heading, message = descriptionWaiting)
						...
						try:
							...
							if not item:
								...
								interface.Core.close()
								return None
							if select: item = self._addSelect(item, category = category, type = type)

							while True:
								if (processing and counter == 5) or (not processing and counter == 10): # Only make an API request every 2.5 or 5 seconds.
									item = self.mDebrid.item(category = category, id = id, title = title, year = year, season = season, pack = pack, strict = strict, episode = episode, transfer = True, files = True)
									if not item:
										self._addError(title = 'Download Error', message = 'Download Failure')
										interface.Core.close()
										return None
								...
								if processing:
									...
									interface.Core.update(progress = 0, title = heading, message = descriptionFinalize)
								elif waiting:
									interface.Core.update(progress = 0, title = heading, message = descriptionWaiting)
								else:
									...
									if percentageNew >= percentage:
										...
										interface.Core.update(progress = int(percentage), title = heading, message = description)
								...
								if (conditionShort or conditionLong) and background:
									...
									answer = interface.Dialog.option(title = heading, message = question, labelConfirm = 'Take Action', labelDeny = 'Continue Download')
									if answer:
				...
				try:
					thread = Pool.thread(target = updateProgress, args = (status, category, id, percentage, close, cached))
					thread.start()
					thread.join()
				except:
					tools.Logger.error()
	...
	def directoryItemAction(self, item):
		...
		if choice >= 0:
			...
			elif choice == 1: network.Networker.linkShow(link = link)
			elif choice == 2: network.Networker.linkOpen(link = link)
					...
					if choice == 'remove':
						...
						if 'video' in item and not item['video'] == None:
							...
							if choice == 'browsecontent':
								...
								if network.Networker.linkIs(itemLink): network.Networker.linkShow(link = itemLink)
								else: raise Exception('Invalid Best Link: ' + str(itemLink))
							elif choice == 'openbest':
								if network.Networker.linkIs(itemLink): network.Networker.linkOpen(link = itemLink)
								else: raise Exception('Invalid Best Link: ' + str(itemLink))
						else:
