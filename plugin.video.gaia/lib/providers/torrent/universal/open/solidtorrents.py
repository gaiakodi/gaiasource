...
#gaiaremove - Both the API and HTML version has been updated, but not yet tested.
#gaiaremove - This was tested with the BitSearch domain, but once the SolidTorrents domain comes online again, this should be tested.
#gaiaremove - Test both version 1 + 2 (API + website). Test with and without an API key.

# This is the exact same site as BitSearch, just with a different domain.
...
# However, sometimes one site is accessible while the other is not. The inaccessible site typically has a Cloudflare configuration error.
...
class Provider(ProviderJson, ProviderHtml):
	...
	def initialize(self):
		...
		# Update (2024-12): Domain is up again, but sometimes a search results in a gateway error.
		# Update (2025-01): The .net domain is completely down, and .eu domain loads a semi-broken website.
		# Update (2025-06): Domains still return errors (too many requests or occasional Cloudflare errors) most of the time, but every now and then a request is successful.
		# Update (2025-12): The .to domain is now completely offline. The .eu domain works now. It was online for a few days. But then it was inaccessible for the past 2+ days (due to Cloudflare 1016 server misconfiguration errors).
		#status					= ProviderJson.StatusOperational
		status					= ProviderJson.StatusImpaired # Down very often with "Too many requests, please try again later." errors or Cloudflare 520 errors, or Cloudflare 1016 server misconfiguration errors.
				...
				accountVerification			= {
												ProviderJson.ProcessRequest : {
													...
													ProviderJson.RequestData : {
														...
														# Either SolidTorrents or Cloudflare is caching the results of a request based on the GET parameters.
