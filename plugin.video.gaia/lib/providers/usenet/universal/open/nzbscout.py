...
class Provider(ProviderUsenetHtml):
	...
	_ExpressionId			= r'.*\/([a-z0-9\-]+)(?:$|\/)'
	_ExpressionSize			= r'size\s*:?\s*(\d+(?:\.\d+)?\s*[kmgt]?b)\s*(?:$|\n)'
	_ExpressionTime			= r'posted\s*:?\s*([\d\-\:\s]+)\s*(?:$|\n)'
	_ExpressionUploader		= r'poster\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionDownloads	= r'downloaded\s*:?\s*([\d\,]+)\s*times?\s*(?:$|\n)'
	_ExpressionVideo		= r'video\s*format\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionCodec		= r'codec\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionResolution	= r'width\s*x\s*height\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionAspect		= r'aspect\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionAudio		= r'audio\s*format\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionChannels		= r'channels\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionLanguage		= r'media\s*info.*language\s*:?\s*(.*?)\s*(?:$|\n)'
	_ExpressionAnonymous	= r'^anonymous$'
