...
try:
	from urllib.parse import urlparse, parse_qsl, quote, quote_plus, unquote, unquote_plus
...
except: from lib.modules.database import Database

try: from convert import ConverterTime, ConverterDuration, ConverterSpeed, ConverterSize
except: from lib.modules.convert import ConverterTime, ConverterDuration, ConverterSpeed, ConverterSize

class Downloader(Database):
	...
	def __init__(self, type = TypeManual, id = None):
		# sys.argv can changen when using <reuselanguageinvoker>.
		self.mAddon = sys.argv[0]
		try: self.mHandle = int(sys.argv[1])
		except: self.mHandle = 0

		self.mType = type
		Database.__init__(self, Downloader.Database, addon = Downloader.AddonId)
	...
	def __delete__(self):
		Database.__delete__(self)

	def _reset(self):
		# Must have a prefix "Download" otherwise mPath clashes with Database's mPath.
	...
	def _run(self, action, observation = True):
		command = 'RunScript(%s, %s, %s, %s, %s, %s, %d)' % ('plugin.video.gaia', '-1', 'action=downloadExecute', quote_plus(action), quote_plus(self.mType), quote_plus(self.mDownloadId), observation)
	...
	def _response(self, link, headers, size, timeout = 30):
		try:
			...
			try:
				...
				return urlopen(request, timeout = timeout, context = secureContext)
			except:
	...
	def _translate(self, string):
		if isinstance(string, int):
			string = xbmcaddon.Addon(Downloader.AddonId).getLocalizedString(string)
		return string
	...
	def _title(self, extension = None, color = None, name = False, bold = True):
		title = xbmcaddon.Addon(Downloader.AddonId).getAddonInfo('name') if name else ''
	...
	def _icon(self, status = None, stalled = None, small = False):
		if self.mIconPath == None:
			theme = self._setting('theme.general.icon', raw = True).lower()
			if 'glass' in theme:
	...
	def _background(self):
		if self.mSkinPath == None:
			theme = self._setting(id = 'theme.general.icon', raw = True)
			if theme in ['default', '-', '']:
	...
	def _notification(self, messages, status = None):
		...
		path = xbmcaddon.Addon('script.gaia.resources').getAddonInfo('path')
	...
	def _setting(self, id, raw = False): # id excluding the prefix download.manual or downloa.cache.
		return xbmcaddon.Addon(Downloader.AddonId).getSetting(self._settingId(id = id, raw = raw))

	def _settingId(self, id, raw = False):
		if not raw: id = '.'.join(['download', self.mType, id])
		return id
	...
	def _locationMovies(self):
		...
		if self._setting('location.selection') == '0':
			path = os.path.join(self._settingPath('location.combined'), self._translate(32001))
	...
	def _locationShows(self):
		path = None
		if self._setting('location.selection') == '0':
			path = os.path.join(self._settingPath('location.combined'), self._translate(32002))
	...
	def _locationOther(self):
		path = None
		if self._setting('location.selection') == '0':
			path = os.path.join(self._settingPath('location.combined'), self._translate(35149))
			try: xbmcvfs.mkdirs(path)
			except: pass
		else:
			path = self._settingPath('location.other')
		if not path.endswith('/') and not path.endswith('\\'): path += '/'
		return path

	def _file(self, title, link):
		extension = os.path.splitext(urlparse(link).path)[1][1:]
		...
		if len(content) == 0:
			...
			show = content[0][0].translate(map).strip('.')
	...
	@classmethod
	def cleanDatabase(self, type = None, confirm = False):
		if type is None:
			self.cleanDatabase(type = Downloader.TypeManual, confirm = confirm)
			self.cleanDatabase(type = Downloader.TypeCache, confirm = confirm)
		else:
			Database(Downloader.Database).clear(tables = type, confirm = confirm)

	@classmethod
	def cleanFile(self, type = None):
		if type is None:
			self.cleanFile(type = Downloader.TypeManual)
			self.cleanFile(type = Downloader.TypeCache)
		else:
			from .tools import File
			downloader = Downloader(type = type)
			if downloader._setting('location.selection') == '0':
				File.deleteDirectory(downloader._settingPath('location.combined'))
			else:
	...
	@classmethod
	def sizeDatabase(self, type = None):
	...
	def speed(self, refresh = False):
		...
		return ConverterSpeed(value = speed, unit = ConverterSpeed.Byte).stringOptimal()
	...
	def _itemsUpdate(self, status, handle, addon, colors):
		...
		for item in result:
			...
			if self.mProgressSizeCompleted > 0:
				size = ConverterSize(value = self.mProgressSizeCompleted).stringOptimal(places = ConverterSize.PlacesDouble)
				if self.mDownloadSize > 0:
					size += ' ' + self._translate(33073) + ' ' + ConverterSize(value = self.mDownloadSize).stringOptimal(places = ConverterSize.PlacesDouble)
				info.append(size)

			# Speed
			if (self.mDownloadStatus == Downloader.StatusRunning or stalledShort) and not stalledLong and self.mProgressSpeed > 0:
				speed = ConverterSpeed(value = self.mProgressSpeed, unit = ConverterSpeed.Byte).stringOptimal()
				info.append(speed)
			...
			if (self.mDownloadStatus == Downloader.StatusRunning or stalledShort) and not stalledLong and self.mDownloadSize > 0 and self.mProgressTimeRemaining > 0:
				timeRemaining = ConverterDuration(value = self.mProgressTimeRemaining, unit = ConverterDuration.UnitSecond).string()
			...
			menu.append((self._translate(33031), 'RunPlugin(%s?action=copy&link=%s)' % (addon, self.mDownloadLink)))
	...
	def items(self, status = None, refresh = True):
		...
		self._itemsUpdate(status, self.mHandle, self.mAddon, colors)
	...
	def details(self):
		...
		if self.mDownloadSize > 0:
			size = ConverterSize(value = self.mDownloadSize).stringOptimal(places = ConverterSize.PlacesDouble)
		else:
			size = ''
		...
		if self.mDownloadTimeStarted == None or self.mDownloadTimeStarted == '':
			...
			timeCreated = ConverterTime(self.mDownloadTimeStarted).string(ConverterTime.FormatDateTime)
		if self.mDownloadTimeUpdated == None or self.mDownloadTimeUpdated == '':
			timeModified = ''
		else:
			timeModified = ConverterTime(self.mDownloadTimeUpdated).string(ConverterTime.FormatDateTime)
		...
		progressSize = ConverterSize(value = self.mProgressSizeCompleted).stringOptimal(places = ConverterSize.PlacesDouble)
		if self.mDownloadSize > 0:
			progressSize += ' ' + self._translate(33073) + ' ' + ConverterSize(value = self.mDownloadSize).stringOptimal(places = ConverterSize.PlacesDouble)
		if self.mDownloadStatus == Downloader.StatusRunning:
			progressSpeed = ConverterSpeed(value = self.mProgressSpeed, unit = ConverterSpeed.Byte).stringOptimal()
			progressTime = ConverterDuration(value = self.mProgressTimeRemaining, unit = ConverterDuration.UnitSecond).string()
		else:
			progressSpeed = ''
			progressTime = ''
	...
	def download(self, media = None, link = None, title = None, image = None, metadata = None, source = None, id = None, forceAction = False, refresh = False, automatic = False):
		...
		if id is None:
			...
			if not title:
				try: title = unquote(self.mDownloadLink.split('/')[-1])
		...
		try:
			...
			try: self.mDownloadHeaders = dict(parse_qsl(link.rsplit('|', 1)[1]))
			except: self.mDownloadHeaders = {}
	...
	def _play(self, buffering = True):
		self._loaderShow()
		if buffering:
			command = 'RunPlugin(plugin://%s/?action=playLocal&type=%s&downloadType=%s&downloadId=%s&path=%s&metadata=%s&source=%s)' % (Downloader.AddonId, self.mDownloadMedia, quote_plus(self.mType), quote_plus(self.mDownloadId), quote_plus(self.mDownloadPath), quote_plus(json.dumps(self.mDownloadMetadata)), quote_plus(json.dumps(self.mDownloadSource)))
		else:
			command = 'RunPlugin(plugin://%s/?action=playLocal&type=%s&path=%s&metadata=%s&source=%s)' % (Downloader.AddonId, self.mDownloadMedia, quote_plus(self.mDownloadPath), quote_plus(json.dumps(self.mDownloadMetadata)), quote_plus(json.dumps(self.mDownloadSource)))
		xbmc.executebuiltin(command)
		...
		while True:
			...
			if self.mDownloadStatus in [Downloader.StatusQueued, Downloader.StatusInitialized]:
				if self.mProgressDialog == None:
					...
					empty = ' '
					self.mProgressDialog.update(progress, Downloader.FontNewline.join([line, empty, empty]))
			elif self.mProgressSpeed > 0:
				progress = int(self.mProgressPercentageCompleted)
				speed = self._fontBold(self._translate(33074) + ': ') + ConverterSpeed(value = self.mProgressSpeed, unit = ConverterSpeed.Byte).stringOptimal()
				size = self._fontBold(self._translate(33075) + ': ') + ConverterSize(value = self.mProgressSizeCompleted).stringOptimal(places = ConverterSize.PlacesDouble)
				...
				if self.mDownloadSize > 0:
					size += ' ' + self._translate(33073) + ' ' + ConverterSize(value = self.mDownloadSize).stringOptimal(places = ConverterSize.PlacesDouble)
					timeString = ConverterDuration(value = self.mProgressTimeRemaining, unit = ConverterDuration.UnitSecond).string()
				...
				if self.mProgressDialog == None:
					...
					self.mProgressDialog.update(progress, Downloader.FontNewline.join([line1, line2, line3]))
				...
				if path.startswith(Downloader.PrefixSamba):
					...
					if not username == None:
						...
						if not password == None:
							command += '%' + password
					process = subprocess.Popen(command, shell = True, stdout = subprocess.PIPE, stderr = subprocess.STDOUT) # Use "shell" to prevent Windows from showing CMD window. Command must be a string, not a list.
	...
	@classmethod
	def execute(self, action, type, id, observation = False):
		action = unquote_plus(action)
		type = unquote_plus(type)
		id = unquote_plus(id)
		observation = observation == 1 or observation == True or observation == '1' or observation.lower() == 'true'
