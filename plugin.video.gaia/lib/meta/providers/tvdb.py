...
class MetaTvdb(MetaProvider):
	...
	def _retrieve(self, media, type, extended = False, translations = False, episodes = False, id = None, level = None):
		if id:
			...
			if translations or episodes:
				...
				if translations and level >= MetaProvider.Level3: data[MetaTvdb.ParameterMeta].append(MetaTvdb.ParameterTranslations)
	...
	def _extractImage(self, data, person = False):
		...
		if link:
			...
			# Maybe this is releated top the update here: https://github.com/thetvdb/v4-api/issues/224
	...
	def _search(self, id = None, idImdb = None, idTmdb = None, idTvdb = None, idTrakt = None, query = None, year = None, numberSeason = None, numberEpisode = None, media = None, limit = None, offset = None, page = None, level = None):
		...
		if data:
			...
			if media in [MetaData.MediaSeason, MetaData.MediaEpisode]:
				...
				show = self.show(idTvdb = show, level = level if level >= MetaProvider.Level4 else MetaProvider.Level4)
				result = []
	...
	def _translation(self, id = None, idImdb = None, idTmdb = None, idTvdb = None, idTrakt = None, query = None, year = None, numberSeason = None, numberEpisode = None, media = None, translation = None, level = None):
		...
		if result:
			if translation == MetaProvider.TranslationTitle: return result.title(language = True)
			elif translation == MetaProvider.TranslationOverview: return result.overview(language = True)
		return None
