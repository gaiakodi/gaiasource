...
import bs4
from externals.beautifulsoup import BeautifulSoup, __version__
from externals.beautifulsoup.builder import builder_registry
from typing import (
    Any,
    IO,
    List,
...
if TYPE_CHECKING:
    from externals.beautifulsoup._typing import _IncomingMarkup
...
def diagnose(data: "_IncomingMarkup") -> None:
    ...
    if "html5lib" in basic_parsers:
        try:
            from externals import html5lib
...
def profile(num_elements: int = 100000, parser: str = "lxml") -> None:
    ...
    data = rdoc(num_elements)
    vars = dict(externals.beautifulsoup.externals.beautifulsoup. data=data, parser=parser)
    cProfile.runctx("externals.beautifulsoup.BeautifulSoup(data, parser)", vars, vars, filename)

    stats = pstats.Stats(filename)
    # stats.strip_dirs()
    stats.sort_stats("cumulative")
    stats.print_stats("_html5lib|externals.beautifulsoup., 50)
