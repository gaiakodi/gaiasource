...
from externals.requests.adapters import HTTPAdapter
from externals.requests.sessions import Session
from externals.requeststoolbelt.utils import dump
...
class CipherSuiteAdapter(HTTPAdapter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.ssl_context:
            ...
            try: self.ssl_context.orig_wrap_socket = self.ssl_context.wrap_socket
            except: self.orig_wrap_socket = self.ssl_context.wrap_socket
            try: self.ssl_context.wrap_socket = self.wrap_socket
            except: pass
        ...
        if self.ssl_verify <= 2:
            ...
            if self.ssl_verify <= 1:
                self.ssl_context.options &= ~ssl.OP_NO_TLSv1_3 & ~ssl.OP_NO_TLSv1_2 & ~ssl.OP_NO_TLSv1_1
                try: self.ssl_context.minimum_version = ssl.TLSVersion.TLSv1
                except: # TLSVersion not known.
                    try: self.ssl_context.minimum_version = ssl.PROTOCOL_TLSv1
    ...
    def wrap_socket(self, *args, **kwargs):
        if hasattr(self.ssl_context, 'server_hostname') and self.ssl_context.server_hostname:
            ...
            self.ssl_context.check_hostname = self.ssl_verify > 2

        # Gaia
        #return self.ssl_context.orig_wrap_socket(*args, **kwargs)
        try: wrap = self.ssl_context.orig_wrap_socket
        except: wrap = self.orig_wrap_socket
...
class CloudScraper(Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mount(
            ...
            CipherSuiteAdapter(
                ...
                ssl_verify=self.ssl_verify # Gaia - Disable SSL
            )
        )
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        try:
            ...
            self.response = response = self.decodeBrotli(
    ...
    def _refresh_session(self, url):
        ...
        try:
            ...
            try:
                ...
                # Gaia - prevent recursive session refreshes.
